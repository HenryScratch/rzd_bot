...
from utils import (add_routes_db, add_user, delete_route_db, get_routes,
                   get_routes_db, parsing_route_db)
...
async def get_routes_state(message: Message, state: State):
    ...
    if len(routes) > 0:
        ...
        await message.answer("Выберите маршрут", reply_markup=await inline_routes(routes))
    else:
...
async def pparsing_route(callback: CallbackQuery, state: FSMContext):
    ...
    if value != None:
        mes = f"{route['station_from']} - {route['station_to']}\nНомер поезда: {route['number_route']}\n{route['time_from']} - {route['time_to']}\n{'\n'.join(( f"{k} - {cleaner(str(v))}" for k, v in route['seats'].items()))}"  
        await callback.message.answer(mes, reply_markup=main_keyboard)
    else:
        await callback.message.answer("Данные о маршруте еще не обновлены", reply_markup=main_keyboard)
...
async def add_src(message: Message, state: FSMContext):
    ...
    if cities != None:
        await message.answer("Выберите из списка", reply_markup=await inline_cities(cities))
        await state.set_state(Route_add.src)
    else:
        await message.answer("Не существует, попробуйте еще раз",)
        await state.set_state(Route_add.input_src)
...
async def add_date_forward(message: Message, state: FSMContext):
    if validate_date(message.text):
        ...
        await message.answer("Введите дату возвращения в формате 01.12.2024 или нажмите 'Пропустить'", reply_markup=skip_keyboard)
    else:
...
async def date_back_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if check:  
        ...
        await callback.message.answer("Выберите маршрут для дальнейшего отслеживания, скопировав и отправив номер поезда")
...
async def get_number_route_single(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    found_route = next((item for item in data['routes'] if item.get('number_route') == message.text), None)
    if found_route != None:
        await state.update_data(route=found_route)
        await message.answer("Выберите один или несколько типов мест для отслеживания или нажмите 'Далее' для выбора всех типов сразу.", reply_markup=await inline_type_seats(found_route['seats'].keys())) 
...
async def type_seats_done_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if len(data['type_seats'])==0:
        ...
        data = await state.get_data()
        await callback.message.edit_text(f"Маршрут: {data['number_route']} добавлен. Выбраны следующие категории: {', '.join(data['type_seats'])}.\nДля более детальной информации о машруте, выберите данный машрут в 'Мои маршруты'.")
...
async def type_seats_selecting_callback(callback: CallbackQuery, state: FSMContext):
    await callback.answer('')
    data = await state.get_data()
    data['type_seats'].add(translit(callback.data, 'ru',))
...
@router.message(F.text == "Отмена")
@router.callback_query(Route_add.type_seats_selecting_from, F.data != 'done')
async def type_seats_selecting_from_callback(callback: CallbackQuery, state: FSMContext):
    await callback.answer('')
    data = await state.get_data()
    data['type_seats'].add(translit(callback.data, 'ru',))
    await state.update_data(type_seats = data['type_seats'])
...
async def add_date_back(message: Message, state: FSMContext):
    if validate_date(message.text):
        ...
        data = await state.get_data()
        data1={}
        data1['src'], data1['dst'], data1['date'] = data['dst'], data['src'], data['date_back']
        data['date'] = data.pop('date_forward')
        data.pop('date_back', None)
        ...
        if check:
            ...
            await message.answer("Выберите маршрут для дальнейшего отслеживания, скопировав и отправив номер поезда")
...
async def delete_route_state(message: Message, state: FSMContext):
    ...
    if len(routes) > 0:
        ...
        await message.answer("Выберите маршрут для удаления", reply_markup=await inline_routes(routes))
    else:
        await message.answer("Нет сохраненных маршрутов")
...
async def delete_route(callback: CallbackQuery, state: FSMContext):
    ...
    if len(routes) > 0:
        await state.set_state(Route_delete.delete)
        await callback.message.edit_text("Выберите маршрут для удаления", reply_markup=await inline_routes(routes))
    else:
